...
const OverviewAttendance = () => {
    return (
        <div className="bg-white rounded-2xl shadow-xl h-full flex flex-col transition hover:shadow-2xl w-full">
            {/* Header */}
            <div className="bg-[#23245c] rounded-t-2xl shadow-xl flex items-center justify-between px-4 md:px-6 py-3 md:py-4">
                <span className="flex items-center gap-2 text-white font-bold text-base md:text-lg tracking-wide">
                    <BadgeCheck className="w-5 h-5 md:w-6 md:h-6 text-[#ffd02c]" />
            ...
            {/* Stats */}
            <div className="flex flex-col md:flex-row flex-1 gap-4 md:gap-6 px-3 md:px-6 py-4 md:py-5">
                {stats.map((item, idx) => (
                    <div
                        key={idx}
                        className={`
                    ...
                    >
                        <div className="flex items-center gap-2 mb-1">
                            <span>{item.icon}</span>
                            <span className="text-[15px] md:text-[16px] font-semibold">{item.title}</span>
                        </div>
                        <CircleProgress
                            total={item.total}
                            color={item.color}
                            absent={item.absent}
                        />
                        <div className="flex justify-between w-full text-[13px] md:text-[14px] px-3 mt-1">
